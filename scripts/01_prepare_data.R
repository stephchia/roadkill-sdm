...
# road data
road.vec <- st_read("data_input/highway/highway.shp") %>% st_transform(crs = 4326)
road <- vect(road.vec) %>% rasterize(rast, touches = TRUE) # rasterize
plot(road)
...
plot(vegetation)
write.csv(veg_levels, "data/var_name.csv")
