...
# road data
road.vec <- st_read("data/input/highway/highway.shp") %>% 
  st_transform(crs = 4326) %>% 
  rasterize(rast, touches = TRUE)
plot(road)
writeRaster(road, "data/processed/road.tif")

## process land cover data
# create raster of taiwan island
taiwan.vec <- st_read("data/input/Taiwan_diss/Taiwan_diss.shp") %>% st_transform(crs = 4326)
taiwan.vec$value <- 1
...
vegetation.vec <- st_read("data/input/vegetation/ACO0301000011021.shp") %>% st_transform(crs = 4326)
...
worldcover <- merge(rast("data/input/landcover/ESA_WorldCover_10m_2021_v200_N21E117_Map.tif"),
                    rast("data/input/landcover/ESA_WorldCover_10m_2021_v200_N21E120_Map.tif"),
                    rast("data/input/landcover/ESA_WorldCover_10m_2021_v200_N24E120_Map.tif")) %>%
...
plot(land)
writeRaster(land, "data/processed/landcover.tif")
...
writeRaster(dist_to_water, "data/processed/dist_to_water.tif")
